...
class DuplicateDataset(Dataset):
    ...
    def _posts_comparison_stats(self, row):
        ...
        row['post1_hashes'] = [] if  row['post1_hashes'] is None else row['post1_hashes']
        row['post2_hashes'] = [] if row['post2_hashes'] is None else row['post2_hashes']
        post1_has_image = len(row['post1_hashes']) > 0
        post2_has_image = len(row['post2_hashes']) > 0
        common_image_hashes = len(set(row['post1_hashes']) & set(row['post2_hashes']))


        post1_has_price = row['post1_price'] is not None 
        post2_has_price = row['post2_price'] is not None 

        post1_price = int(row['post1_price']) if post1_has_price else 0
        post2_price = int(row['post2_price']) if post2_has_price else 0

        same_price = post1_price == post2_price

        price_diff = np.float(np.abs(post1_price - post2_price) / np.max([post1_price, post2_price, 1]))


        post1_title_words = set(row['post1_title'].split())
        post1_desc_words = set(row['post1_desc'].split())
        post2_title_words = set(row['post2_title'].split())
        post2_desc_words = set(row['post2_desc'].split())

        post1_title_len = len(post1_title_words)
        post2_title_len = len(post2_title_words)

        post1_desc_len = len(post1_desc_words)
        post2_desc_len = len(post2_desc_words)

        titles_intersecion_len = len(post1_title_words & post2_title_words)
        descs_intersecion_len = len(post1_desc_words & post2_desc_words)
        texts_intersecion_len = len((post1_title_words | post1_desc_words) & (post2_title_words | post2_desc_words))

        titles_union_len = len(post1_title_words | post2_title_words)
        descs_union_len = len(post1_desc_words | post2_desc_words)
        texts_union_len = len((post1_title_words | post1_desc_words) | (post2_title_words | post2_desc_words))

        titles_P1_diff_P2_len = len(post1_title_words - post2_title_words)
        descs_P1_diff_P2_len = len(post1_desc_words - post2_desc_words)
        texts_P1_diff_P2_len = len((post1_title_words | post1_desc_words) - (post2_title_words | post2_desc_words))

        titles_P2_diff_P1_len = len(post2_title_words - post1_title_words)
        descs_P2_diff_P1_len = len(post2_desc_words - post1_desc_words)
        texts_P2_diff_P1_len = len((post2_title_words | post2_desc_words) - (post1_title_words | post1_desc_words))
        ...
        stats = [
            ...
            post1_desc_len,
            post2_desc_len,
            titles_intersecion_len,
            descs_intersecion_len,
            texts_intersecion_len,
            titles_union_len,
            descs_union_len,
            texts_union_len,
            titles_P1_diff_P2_len,
            descs_P1_diff_P2_len,
            texts_P1_diff_P2_len,
            titles_P2_diff_P1_len,
            descs_P2_diff_P1_len,
            texts_P2_diff_P1_len,
        ]
    ...
    def __getitem__(self, item):
        ...
        post1_x = [post1_cat_slug, post1_city, post1_neighbor, post1_title, post1_description]
        post2_x = [post2_cat_slug, post2_city, post2_neighbor, post2_title, post2_description]
        y_duplicate = row["is_duplicate"]

        stats = self._posts_comparison_stats(row)
...
class DuplicateDataLoader(pl.LightningDataModule):
    ...
    def collate_batch(self, batch):
        ...
        # Post1 Encodings
        post1_titles_padded, post1_titles_masks = self.encode_texts(post1_titles)
        ...
        post1_x = [post1_titles_padded, post1_titles_masks, post1_descs_padded, post1_descs_masks, post1_slugs_encoded, post1_cities_encoded, post1_neighbors_encoded]

        # Post2 Encodings
        post2_titles_padded, post2_titles_masks = self.encode_texts(post2_titles)
        ...
        post2_x = [post2_titles_padded, post2_titles_masks, post2_descs_padded, post2_descs_masks, post2_slugs_encoded, post2_cities_encoded, post2_neighbors_encoded]
