...
def presicion_recall(y_true, y_pred):
    ...
    threshold = 0.75
    print("duplicate count: ", np.unique(
        np.array(y_pred) >= threshold, return_counts=True))
...
def calculate_roc(row):
    ...
    try:
        ...
        auc = np.nan
    try:
        precision = presicion_recall(
            y_true=row['is_duplicate'], y_pred=row['prediction'])[0]
    except:
        precision = []
...
precision, recall_neg, recall, thresholds, auc = presicion_recall(
    test_data["is_duplicate"], test_data['prediction'])
